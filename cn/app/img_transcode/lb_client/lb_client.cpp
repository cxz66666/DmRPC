...
void handler_ping(ClientContext *ctx, REQ_MSG req_msg)
{
    ...
    new (ctx->ping_msgbuf.buf_) PingReq(RPC_TYPE::RPC_PING, req_msg.req_id, FLAGS_extra_flags);
...
void handler_tc(ClientContext *ctx, REQ_MSG req_msg)
{
    ...
    new (req_msgbuf.buf_) TranscodeReq(RPC_TYPE::RPC_TRANSCODE, req_msg.req_id, req_msgbuf.get_data_size() - sizeof(TranscodeReq), FLAGS_extra_flags);
...
void client_thread_func(size_t thread_id, ClientContext *ctx, erpc::Nexus *nexus)
{
    ...
    {
        // TODO
        ctx->req_msgbuf[i] = rpc.alloc_msg_buffer_or_die(sizeof(TranscodeReq) + FLAGS_block_size);
...
void server_thread_func(size_t thread_id, ServerContext *ctx, erpc::Nexus *nexus)
{
    ...
    while (true)
    {
        ...
        printf("client thread %zu: ping_req : %.2f, ping_resp : %.2f, tc : %.2f, tc_req : %.2f Gb \n", thread_id,
               ctx->stat_req_ping_tot / seconds, ctx->stat_req_ping_resp_tot / seconds, ctx->stat_req_tc_tot / seconds, ctx->stat_req_tc_req_tot * 8 * FLAGS_block_size / (GB(1) * seconds));
    ...
    if (connected)
    {
        ...
        total_speed += std::accumulate(resp_total.begin(), resp_total.end(), 0.0) * 8 * FLAGS_block_size * 1e6 / (GB(1) * now_timer.toc());
