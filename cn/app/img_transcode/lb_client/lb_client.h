...
DEFINE_uint64(block_size, 4096, "block size for test");
DEFINE_uint64(extra_flags, 0, "extra flags for test");
...
public:
    ...
    {
        spsc_queue = new atomic_queue::AtomicQueueB2<REQ_MSG, std::allocator<REQ_MSG>, true, false, false>(kAppMaxConcurrency);
    }
    ~ClientContext()
    {
        delete spsc_queue;
...
public:
    ...
    {
        resp_spsc_queue = new atomic_queue::AtomicQueueB2<RESP_MSG, std::allocator<RESP_MSG>, true, false, false>(kAppMaxConcurrency);
    }
    ~ServerContext()
    {
    ...
    // used for tc send
    atomic_queue::AtomicQueueB2<REQ_MSG, std::allocator<REQ_MSG>, true, false, false> *spsc_queue{nullptr};
