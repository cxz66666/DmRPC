...
#include "img_bitmap_worker_cxl.h"
...
void worker_thread_func(size_t thread_id)
{
    ...
    while (true)
    {
        for (size_t i = thread_id; i < rpc_connect_number; i += FLAGS_worker_num)
        {
            ...
            size_t num = forward_spsc_queue[i]->was_size();
            for (size_t j = 0; j < num; j++)
            {
                ...
                // cxls_[i].seekg(req->extra.offset, std::ios_base::beg);
                // cxls_[i].read(reinterpret_cast<char *>(cxl_req_msgbuf[rpc_id][req_id]), req->extra.length);
                memcpy(cxl_req_msgbuf[rpc_id][req_id], static_cast<char *>(cxls_addr_[i]) + req->extra.offset, req->extra.length);

                if (unlikely(!resize_bitmap(cxl_req_msgbuf[rpc_id][req_id], cxl_resp_msgbuf[rpc_id][req_id])))
                {
