...
void handler_ping(ClientContext *ctx, REQ_MSG req_msg)
{
    ...
    CxlParam &param = ctx->cxl_params_[rpc_id];
...
{
    uint32_t rpc_id = req_msg.req_id;
    uint32_t req_id = ctx->cxl_req_ids_[rpc_id]++;
    ...
    ctx->rpc_->enqueue_request(ctx->session_num_vec_[0], static_cast<uint8_t>(RPC_TYPE::RPC_TRANSCODE),
                               ...
                               callback_tc, reinterpret_cast<void *>(ctx->cxl_flags_[rpc_id] | req_id_mod));
}
...
void leader_thread_func()
{
    ...
    for (size_t i = 0; i < FLAGS_server_num; i++)
    {
        size_t total_ping_size = context->client_contexts_[i]->cxl_params_.size();
...
int main(int argc, char **argv)
{
    ...
    // with file_size byte and (file_size_aligned - file_size) zero byte
    file_buf = new uint8_t[file_size_aligned];
