...
void connect_sessions(ClientContext *c)
{
    ...
    std::vector<size_t> forward_server_ports = flags_get_balance_servers_index();
    std::vector<size_t> backward_server_ports = flags_get_balance_back_index();
    rmem::rt_assert(forward_server_ports.size() == backward_server_ports.size(), "forward_server_ports.size() != backward_server_ports.size()");
    ...
    for (auto m : backward_server_ports)
    {
        std::string remote_uri = rmem::get_uri_for_process(m);
        ...
        c->backward_session_nums_.push_back(session_num_backward);
    }
...
void transcode_handler(erpc::ReqHandle *req_handle, void *_context)
{
    ...
    new (req_handle->pre_resp_msgbuf_.buf_) TranscodeResp(req->req.type, req->req.req_number, 0, req->extra.length, req->extra.flags);
    ...
    if (likely(ctx->req_forward_msgbuf_ptr[req->extra.flags][req->req.req_number % kAppMaxBuffer].buf_ != nullptr))
    {
        ctx->rpc_->free_msg_buffer(ctx->req_forward_msgbuf_ptr[req->extra.flags][req->req.req_number % kAppMaxBuffer]);
    }
...
void transcode_resp_handler(erpc::ReqHandle *req_handle, void *_context)
{
    ...
    if (ctx->req_backward_msgbuf_ptr[req->extra.flags][req->req.req_number % kAppMaxBuffer].buf_ != nullptr)
    {
        ctx->rpc_->free_msg_buffer(ctx->req_backward_msgbuf_ptr[req->extra.flags][req->req.req_number % kAppMaxBuffer]);
    }
...
void handler_ping(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf)
{
    ...
    ctx->req_forward_msgbuf[req->timestamp][req->req.req_number % kAppMaxBuffer] = req_msgbuf;

    erpc::MsgBuffer &resp_msgbuf = ctx->resp_forward_msgbuf[req->timestamp][req->req.req_number % kAppMaxBuffer];

    // TODO load balance?

    size_t session_num = 0;
#if defined(ERPC_PROGRAM)
    session_num = ctx->session_num_vec_[req->timestamp];
#elif defined(RMEM_PROGRAM)
    ...
    ctx->rpc_->enqueue_request(session_num, static_cast<uint8_t>(RPC_TYPE::RPC_PING),
                               &ctx->req_forward_msgbuf[req->timestamp][req->req.req_number % kAppMaxBuffer], &resp_msgbuf,
                               callback_ping, reinterpret_cast<void *>((req->timestamp << 32) | (req->req.req_number % kAppMaxBuffer)));
}
...
void callback_ping_resp(void *_context, void *_tag)
{
    ...
    erpc::MsgBuffer &resp_msgbuf = ctx->resp_backward_msgbuf[flags][req_id];
...
void handler_ping_resp(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf)
{
    ...
    ctx->req_backward_msgbuf[req->timestamp][req->req.req_number % kAppMaxBuffer] = req_msgbuf;

    erpc::MsgBuffer &resp_msgbuf = ctx->resp_backward_msgbuf[req->timestamp][req->req.req_number % kAppMaxBuffer];

    ctx->rpc_->enqueue_request(ctx->backward_session_nums_[req->timestamp], static_cast<uint8_t>(RPC_TYPE::RPC_PING_RESP),
                               &ctx->req_backward_msgbuf[req->timestamp][req->req.req_number % kAppMaxBuffer], &resp_msgbuf,
                               callback_ping_resp, reinterpret_cast<void *>((req->timestamp << 32) | (req->req.req_number % kAppMaxBuffer)));
}
...
void handler_tc(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf)
{
    ...
    ctx->req_forward_msgbuf[req->extra.flags][req->req.req_number % kAppMaxBuffer] = req_msgbuf;

    erpc::MsgBuffer &resp_msgbuf = ctx->resp_forward_msgbuf[req->extra.flags][req->req.req_number % kAppMaxBuffer];

    size_t session_num = 0;
#if defined(ERPC_PROGRAM)
    session_num = ctx->session_num_vec_[req->extra.flags];
#elif defined(RMEM_PROGRAM)
    ...
    ctx->rpc_->enqueue_request(session_num, static_cast<uint8_t>(RPC_TYPE::RPC_TRANSCODE),
                               &ctx->req_forward_msgbuf[req->extra.flags][req->req.req_number % kAppMaxBuffer], &resp_msgbuf,
                               callback_tc, reinterpret_cast<void *>((req->extra.flags << 32) | (req->req.req_number % kAppMaxBuffer)));
}
...
void handler_tc_resp(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf)
{
    ...
    ctx->req_backward_msgbuf[req->extra.flags][req->req.req_number % kAppMaxBuffer] = req_msgbuf;

    erpc::MsgBuffer &resp_msgbuf = ctx->resp_backward_msgbuf[req->extra.flags][req->req.req_number % kAppMaxBuffer];

    ctx->rpc_->enqueue_request(ctx->backward_session_nums_[req->extra.flags], static_cast<uint8_t>(RPC_TYPE::RPC_TRANSCODE_RESP),
                               &ctx->req_backward_msgbuf[req->extra.flags][req->req.req_number % kAppMaxBuffer], &resp_msgbuf,
                               callback_tc_resp, reinterpret_cast<void *>((req->extra.flags << 32) | (req->req.req_number % kAppMaxBuffer)));
}
...
void client_thread_func(size_t thread_id, ClientContext *ctx, erpc::Nexus *nexus)
{
    ...
    ctx->rpc_ = &rpc;
    for (size_t i = 0; i < kAppMaxNum; i++)
    {
        for (size_t j = 0; j < kAppMaxBuffer; j++)
        {
            // TODO
            ctx->resp_forward_msgbuf[i][j] = rpc.alloc_msg_buffer_or_die(sizeof(TranscodeResp));
            ctx->resp_backward_msgbuf[i][j] = rpc.alloc_msg_buffer_or_die(sizeof(TranscodeResp));
        }
    }
...
void server_thread_func(size_t thread_id, ServerContext *ctx, erpc::Nexus *nexus)
{
    ...
    while (true)
    {
        ...
        printf("lb thread %zu: ping_req : %.2f, ping_resp : %.2f, tc : %.2f, tc_req : %.2f \n", thread_id,
