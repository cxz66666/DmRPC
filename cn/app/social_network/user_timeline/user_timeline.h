...
void read_post_details(void *buf_, erpc::Rpc<erpc::CTransport> *rpc_, MPMC_QUEUE *consumer_fwd, MPMC_QUEUE *consumer_back) {
    ...
    if(req->req_control.stop > static_cast<int>(post_ids.size())){
        is_fwd = false;
    }

    if(!is_fwd){
        erpc::MsgBuffer resp_buf = rpc_->alloc_msg_buffer_or_die(sizeof(RPCMsgReq<UserTimeLineReq>));
        auto* resp = new (resp_buf.buf_) RPCMsgReq<UserTimeLineReq>(RPC_TYPE::RPC_USER_TIMELINE_READ_RESP, req->req_common.req_number,
    ...
    int now_index = 0;
    for(auto m=post_ids.begin(); m!=post_ids.end(); m++){
        if(req->req_control.start<=now_index && now_index<req->req_control.stop){
            post_storage_req.add_post_ids(*m);
        }
        now_index++;
    ...
    post_storage_req.SerializeToArray(fwd_req_msg+1,post_storage_req.ByteSizeLong());
