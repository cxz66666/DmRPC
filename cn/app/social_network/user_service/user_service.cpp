...
void compose_creator_with_user_id_handler(erpc::ReqHandle *req_handler, void *_context)
{
    auto *ctx = static_cast<ServerContext *>(_context);
    ctx->stat_req_compose_creator_with_user_id_tot++;
    ...
    social_network::Creator creator_req;
    creator_req.ParseFromArray(req+1,static_cast<int>(req->req_control.data_length));
    rmem::rt_assert(creator_req.IsInitialized(),"req parsed error");

    social_network::Creator creator_resp;
    creator_resp = creator_req ;

    size_t resp_data_length = creator_resp.ByteSizeLong();

    rmem::rt_assert(resp_data_length< 1000, "creator_resp too large");
    ...
    creator_resp.SerializeToArray(req_handler->pre_resp_msgbuf_.buf_ + sizeof(RPCMsgResp<CommonRPCResp>), resp_data_length);
...
void server_thread_func(size_t thread_id, ServerContext *ctx, erpc::Nexus *nexus)
{
    ...
    while (true)
    {
        ...
        printf("thread %zu: ping_req : %.2f, compose_creator_req : %.2f \n", thread_id,
               ctx->stat_req_ping_tot / seconds, ctx->stat_req_compose_creator_with_user_id_tot / seconds);
...
void leader_thread_func()
{
    ...
    nexus.register_req_func(static_cast<uint8_t>(RPC_TYPE::RPC_COMPOSE_CREATOR_WITH_USER_ID), compose_creator_with_user_id_handler);
