...
async def upload_compose(session, addr, user_id, num_users):
    text = ''.join(random.choices(string.ascii_letters + string.digits, k=PERSON_COMPOSE_SIZE))
    # user mentions
    for _ in range(PERSON_USER_MENTIONS):
        text += ' @username_' + str(random.randint(0, num_users))
    # urls
    for _ in range(PERSON_URLS):
        text += ' http://' + \
                ''.join(random.choices(string.ascii_lowercase + string.digits, k=PERSON_URL_SIZE))
    # media
    media_ids = []
    media_types = []
    for _ in range(MEDIA_NUMBER):
        media_ids.append('\"' + ''.join(random.choices(string.digits, k=MEDIA_ID_SIZE)) + '\"')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compose', action='store_true',
                        help='intialize with up to 10 posts per user', default=True)
