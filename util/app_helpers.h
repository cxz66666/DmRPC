...
namespace rmem
{
    ...
    static std::string get_line_n(std::string filename, size_t n)
    {
        ...
        rt_assert(!in.fail(), "Required file " + filename + " not found");
        std::string s;

        for (size_t i = 0; i < n; i++)
        {
            std::getline(in, s);
            rt_assert(!s.empty(), "Insufficient lines in " + filename);
        }
    ...
    {
        std::string process_file = "../cn/app/app_process_file";
    ...
    {
        std::string hostname = get_hostname_for_process(process_i);
        std::string udp_port_str = get_udp_port_for_process(process_i);
