...
user = "cxz"
passwd = "cxz123"
output_file_format = "/home/cxz/cxl_uncore_result/{}_b{}_t{}_c{}_core{}"
msg_size = [4096, 8192, 16384, 32768, 65536, 131072, 262144]
...
num_threads = [5]
core_freq = ["0xc18", "0x0c0c", "0x0b0b", "0x0a0a", "0x0909", "0x0808", "0x0707"]
...
def make_and_clean(ssh):
    stdin, stdout, stderr = ssh.exec_command(
        "cd {0} && echo img_transcode > ./cn/app/build_app && echo cxl > ./cn/app/img_transcode/build_app  && cp "
        "./cn/app/app_process_file_cxl ./cn/app/app_process_file && rm -rf "
...
def uncore_run(ssh, freq):
    stdin, stdout, stderr = ssh.exec_command(
        "sudo wrmsr -a 0x620 {0} && sleep 1".format(freq), get_pty=True
    )
    str1 = stdout.read().decode('utf-8')
    str2 = stderr.read().decode('utf-8')
    print(str1)
...
extra_client = "--test_loop=20 --concurrency={0} --test_bitmap_file={1} --cxl_fake_folder=/testcxl --latency_file={2} " \
               "--bandwidth_file={3} --cxl_session_num={4} --cxl_concurrency_scale=100 "
extra_worker = "--resize_factor=0.1 --worker_num={0} --numa_worker_node={1} --worker_bind_core_offset={2}"
            ...
            for c_i in concurrency:
                for freq in core_freq:    
                    if c_i > max_concurrency[index] or c_i < min_concurrency[index]:
                        ...
                        continue
                    f0 = threading.Thread(target=uncore_run,
                                        args=(
                                            ssh_client, freq
                                        ))
                    f0.start()
                    ...
                    t0 = threading.Thread(target=common_run,
                                        args=(
                                            ssh_client, "client_cxl", 0, 1, 0,
                                            extra_client.format(c_i, file_map[m_i],
                                                                output_file_format.format("lat", m_i, t_i, c_i, freq),
                                                                output_file_format.format("band", m_i, t_i, c_i, freq),
                                                                t_i)
                                        ))
                    t5 = threading.Thread(target=common_run,
                                        args=(
                                            ssh_worker1, "img_bitmap_worker_cxl", 5, 1, 2, extra_worker.format(t_i, 0, 2))
                                        )
                    t6 = threading.Thread(target=common_run,
                                        args=(
                                            ssh_worker2, "img_bitmap_worker_cxl", 6, 1, 14,
                                            extra_worker.format(t_i, 0, 2 + t_i))
                                        )
