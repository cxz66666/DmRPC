...
user = "cxz"
passwd = "cxz123"
output_file_format = "/home/cxz/fork_test_cxl_uncore_result/{}_w{}_core{}_cow{}"
msg_size = 32768
zero_copys = [0]
        ...
        for freq in core_freq:
            for z_i in zero_copys:
                ...
                t0 = threading.Thread(target=client_run,
                                        args=(
                                            ssh_client, "fork_test_cxl_client", 0, 1, 0, z_i, extra_client.format(
                                                output_file_format.format("lat", w_i, freq, z_i),
                                                output_file_format.format("bw", w_i, freq, z_i),
                                                msg_size
                                            )))
                t1 = threading.Thread(target=server_run,
                                        args=(
                                            ssh_server, "fork_test_cxl_server", 1, num_threads, 6, z_i,
                                            extra_server.format(msg_size, w_i, write_page_size)))
                t1.start()
                time.sleep(2)
                t0.start()
                t0.join()
                t1.join()
                print("finish: thread {} cow {} num_write {}  core {}\n".format(num_threads, z_i, w_i, freq))
